...
pub fn debug_ui(
    ...
    if let Some(sel) = selected {
        if let Ok((e, name, wp, orbit, statevec, guidance, body, maneuvers, _appearance, tier)) =
            entities.get(sel)
        {
            ...
            if let Some(o) = orbit {
                ...
                let parent_label = entities
                    .get(o.parent)
                    .ok()
                    .map(|(pe, pn, _, _, _, _, _, _, _, _)| {
                        pn.map(|n| n.as_str().to_string())
    ...
    egui::SidePanel::left("debug_tools")
        .resizable(false)
        .default_width(120.0)
        .show(ctx, |ui| {
    ...
    egui::SidePanel::right("debug_entities")
        .default_width(220.0)
        .show(ctx, |ui| {
            ui.label("entities");
            egui::ScrollArea::vertical().show(ui, |ui| {
                for (e, label) in &items {
                    ...
                    if resp.double_clicked() {
                        focus_double_click = Some(*e);
                    } // double click changes focus
                }
            });
        });
    ...
    if spawn_open {
        egui::Window::new("Spawn")
            ...
            .show(ctx, |ui| {
                ...
                ui.horizontal(|ui| {
                    ui.label("name");
                    ui.text_edit_singleline(&mut sf.name);
                });
                ...
                ui.horizontal(|ui| {
                    ui.label("a");
                    ui.add(egui::DragValue::new(&mut sf.a).speed(1.0));
                });
                if sf.kind == SpawnKind::Body {
                    ui.horizontal(|ui| {
                        ui.label("mu");
                        ui.add(egui::DragValue::new(&mut sf.mu).speed(1e-6));
                    });
                }
                ui.horizontal(|ui| {
                    ui.label("mesh r");
                ...
                if sf.advanced {
                    ui.horizontal(|ui| {
                        ui.label("e");
                        ui.add(egui::DragValue::new(&mut sf.e).speed(0.001));
                        ui.label("i");
                        ui.add(egui::DragValue::new(&mut sf.i).speed(0.01));
                    });
                    ui.horizontal(|ui| {
                        ui.label("lan");
                        ui.add(egui::DragValue::new(&mut sf.lan).speed(0.01));
                        ui.label("arg_pe");
                        ui.add(egui::DragValue::new(&mut sf.arg_pe).speed(0.01));
                        ui.label("m0");
                        ui.add(egui::DragValue::new(&mut sf.m0).speed(0.01));
                    });
                }

                let enabled = sf.parent.is_some();
                if ui
                    .add_enabled(enabled, egui::Button::new("spawn"))
                    .clicked()
                {
                    do_spawn = true;
                }
        ...
        egui::Window::new("Mission")
            ...
            .show(ctx, |ui| {
                ui.horizontal(|ui| {
                    ...
                    if ui
                        .button(if in_edit { "▶ Run" } else { "⏸ Edit" })
                        .clicked()
                    {
                ...
                if in_edit && let Some(ee) = edit_el.as_mut() {
                    ...
                    ui.horizontal(|ui| {
                        ...
                        ui.label("e");
                        el_changed |= ui
                            .add(egui::DragValue::new(&mut ee.e).speed(0.001))
                            .changed();
                    });
                    ui.horizontal(|ui| {
                        ui.label("i");
                        el_changed |= ui
                            .add(egui::DragValue::new(&mut ee.i).speed(0.01))
                            .changed();
                        ui.label("lan");
                        el_changed |= ui
                            .add(egui::DragValue::new(&mut ee.lan).speed(0.01))
                            .changed();
                    });
                    ui.horizontal(|ui| {
                        ui.label("arg_pe");
                        el_changed |= ui
                            .add(egui::DragValue::new(&mut ee.arg_pe).speed(0.01))
                            .changed();
                        ui.label("m0");
                        el_changed |= ui
                            .add(egui::DragValue::new(&mut ee.m0).speed(0.01))
                            .changed();
                    });
                }
                ...
                if let Some(m) = last_mission {
                    ...
                    ui.label(format!(
                        "Δv = {:.5} + {:.5} = {:.5}",
                        m.dep_dv, m.capture_dv, m.total_dv
                    ));
                    ui.label(format!("capture t={:.0}", m.t_peri));
                    ui.label(format!(
                        "orbit a={:.3}  e={:.4}  (r_p {:.3})",
                        m.captured_a, m.captured_e, m.r_p
                    ));
                } else {
    ...
    if let Some(target) = selected
        && (pending.is_some() || clear_queue || remove_index.is_some())
        ...
        && let Ok((_, _, _, _, _, _, Some(pbody), _, _, _)) = entities.get(parent)
    {
        ...
        let appearance = Appearance::Sphere {
            radius: sf.mesh_radius,
            color: sf.color,
        };
...
pub fn fps_overlay(mut contexts: EguiContexts, diagnostics: Res<DiagnosticsStore>) -> Result {
    let ctx = contexts.ctx_mut()?;
